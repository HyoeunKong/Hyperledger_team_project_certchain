$(document).ready(function () {
    ...
    $("#dup_check_btn").click(function () {
        ...
        if (email) {
            ...
            $.post("/dup_check", send_params, function (data, status) {
                ...
                const msg_fail = "<p style='color:red; font-size:14px; font-weight:bold'>중복된 이메일이 있습니다.</p>";
                const msg_sucess = "<p style='color:blue; font-size:14px; font-weight:bold'>가입 가능한 이메일입니다.</p>";
                const token_input = `<div style='height: 75px; margin:0px'>
                                        <div class='md-form form-sm mb-5' style='width:78% ; display: inline-block; '>
                                            <i class='fas fa-envelope prefix'></i>
                                            <input type='text' id='token_input' class='form-control form-control-sm validate' placeholder='메일로 전송된 인증번호를 입력하세요.'>
                                            <label data-error='wrong' data-success='right' for='modalLRInput12'></label>
                                        </div>
                                        <button class='btn btn-info' id='token_input_btn' style='font-size: 14px; width:110px; height: 40px; margin: 15px; right: 0px; display: inline-block;'>인증</button>
                                    </div>`;
                if (parsedData.result > 0) {
                    $("#alert_duplication").html(msg_fail);
                    $("#register_email").html("");
                } else {
                    $("#alert_duplication").html(msg_sucess);
                    $("#auth_email").html(token_input);
                    $("#register_email").html("");
                }
    ...
    $(document).on("click", "#token_input_btn", function () {
        ...
        if (token) {
            ...
            $.post("/auth_mail", send_params, function (data, status) {
                const parsedData = JSON.parse(data);
                const msg_fail = "인증번호를 잘못 입력하셨습니다.";
                const msg_sucess = "<p style='color:blue; font-size:14px; font-weight:bold'>정상적으로 인증되었습니다.</p>";
                if (parsedData.result == 0) {
                    alert(msg_fail);
                } else {
                    $("#auth_email").html(msg_sucess);
                }
                alert(JSON.parse(data).msg);
            });
        } else {
    ...
    function KeyCheckResult(parsedData) {
        ...
        for (x in parsedData[1]) {
            ...
            result +=
                ...
                "<!-- Post Meta -->" +
                "<div class='post-meta'>" +
                "<a class='post-author'>발급일 : " + Date(parsedData[1][x].create_at) + "</a>" +
        ...
        for (x in parsedData[2]) {
            result +=
                ...
                "<!-- Post Meta -->" +
                "<div class='post-meta'>" +
                "<a class='post-author'>발급일 : " + Date(parsedData[2][x].create_at) + "</a>" +
